...
export async function fetchEntities(category?: string): Promise<EntityTrackingDoc[]> {
  const url = category ? `${API_BASE}/entities?category=${category}` : `${API_BASE}/entities`;
...
export async function approveImage(entityId: string, model: string, version: number) {
